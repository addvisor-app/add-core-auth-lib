    ...
    if(authClient._ignoreAuthByUri(req.originalUrl)){
        ...
        if(authorized){
            ...
            if(!code){
                ...
                const maxAge = (expireTime) ? expireTime - new Date().getTime() : 1 * 60 * 60 * 1000;
                authClient._log("JWT expires_in:", jwt.expires_in, "expires:", jwt.expires, "expireTime", expireTime, "maxAge:",maxAge);
                
                res.cookie('x-access-token', jwt.access_token  , { maxAge: maxAge, httpOnly: true });
