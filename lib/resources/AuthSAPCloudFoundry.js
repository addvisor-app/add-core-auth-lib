...
const AuthAbstract = require('./AuthAbstract.js')
...
module.exports = class AuthSAPCloudFoundry extends AuthAbstract{
    ...
    async isValidToken (token) {
        ...
        console.log('AuthSAPCloudFoundry isValidToken token:', token);
        if(token){
            //let expDate = new Date(token.exp * 1000);
            //console.log('AuthSAPCloudFoundry isValidToken token dates', expDate, Date.now());
            //console.log('AuthSAPCloudFoundry isValidToken token client_id', token.client_id);
            //if(token.client_id === this._xsauu.clientid && expDate >= Date.now()){
                valid = true;
            //}
        }
        console.log('AuthSAPCloudFoundry isValidToken valid:', valid);
        return valid;
    }
    ...
    async extractToken(redirect, code){
       ...
       for (var property in form) {
        ...
        console.log('AuthSAPCloudFoundry extractToken basic base64', basicAuth64);
    
        let urlToken = this._xsauu.url+'/oauth/token';
        console.log('AuthSAPCloudFoundry extractToken urlToken', urlToken);
        ...
        return new Promise((resolve, reject) => {
            fetch(urlToken, {
                ...
                console.log('AuthSAPCloudFoundry extractToken response ok', response.ok, response.status, response.statusText);
                if(response.ok){
                ...
                console.log('AuthSAPCloudFoundry extractToken err', err);
                reject(err);
            });
        });
    }
